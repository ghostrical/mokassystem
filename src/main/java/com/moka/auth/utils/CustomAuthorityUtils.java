...
public class CustomAuthorityUtils {
   // @Value("${mail.address.admin}")
    ...
    private final List<String> ADMIN_ROLES_STRING = List.of("ADMIN", "CLIENT_USER", "SUPPLY_USER", "FACTORY_USER");
    ...
    public List<GrantedAuthority> createAuthorities(String persnlTypeCode) {
        if (persnlTypeCode.equals(adminPersnlTypeCode)) {
            return ADMIN_ROLES;
        }
        if(persnlTypeCode.equals("PTC001")){
            return CLIENT_USER_ROLES;
        }
        if(persnlTypeCode.equals("PTD0001")){
